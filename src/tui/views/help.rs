...
impl HelpModal {
    ...
    fn journal_help(&self, theme: &Theme) -> Vec<Line<'static>> {
        vec![
            ...
            Self::key_line("  l", "Toggle splits view (ledger only)", theme),
            ...
            Line::from(""),
            Self::section_label("  Splits View (ledger view only):", theme),
            Line::from("  Press 'l' to show one row per split (journal line)"),
            Line::from("  in this account, rather than one row per entry."),
            Line::from("  Select splits (Space / Shift+↑↓) and press 'a' to"),
            Line::from("  reassign many splits from different transactions at"),
            Line::from("  once — ideal for categorizing Uncategorized splits."),
            Line::from(""),
